...
export async function verifyV9AnimationBlending(options = {}) {
  ...
  const blendCheck = verifyBlendWeights(web);
  if (blendCheck !== undefined) {
    comparison.diagnostics.push(blendCheck);
    comparison.firstMismatch ??= blendCheck;
...
async function runWebTrace(root, bundlePath) {
  ...
  if (result.diagnostics.length > 0) {
    throw new Error(result.diagnostics[0]?.message ?? "V9 animation blending web trace failed.");
  }
  ...
  return {
    diagnostics:
      firstMismatch === undefined
        ? []
        : [{ code: "TN_VERIFY_V9_ANIMATION_BLEND_MISMATCH", message: firstMismatch.message, path: firstMismatch.path, severity: "error" }],
    firstMismatch,
...
function mismatch(path, expected, actual) {
  return {
    actual,
    expected,
    message: `V9 animation blending trace mismatch at ${path}: expected ${JSON.stringify(expected)} but received ${JSON.stringify(actual)}.`,
    path,
  };
}
...
function normalizeServiceLog(log) {
  return {
    entries: (log.entries ?? [])
      .filter((entry) => entry.kind === "service" && (entry.service === "animation.play" || entry.service === "animation.query"))
      .map((entry) => ({
        kind: entry.kind,
...
function serviceOrder(service) {
  return ["animation.play", "animation.query"].indexOf(service);
}

function verifyBlendWeights(log) {
  const blend = log.entries
    .map((entry) => entry.payload?.result?.blend)
    .find((candidate) => candidate !== undefined);
  if (blend?.fromClip === "idle" && blend?.toClip === "run" && blend?.fromWeight === 0.5 && blend?.toWeight === 0.5 && blend?.elapsedSeconds === 0.2) {
    return undefined;
  }
  return {
    code: "TN_VERIFY_V9_ANIMATION_BLEND_WEIGHTS_MISSING",
    message: `V9 animation blending trace did not include expected idle->run half-duration weights: ${JSON.stringify(blend)}.`,
    path: "entries/*/payload/result/blend",
    severity: "error",
  };
}
...
async function main() {
  const result = await verifyV9AnimationBlending({
    artifactDir: process.argv[3],
    bundlePath: process.argv[2],
  });
  if (result.ok) {
    process.stdout.write(`V9 animation blending trace passed. Report: ${result.artifacts.diffPath}\n`);
  } else {
    process.stderr.write(`${result.comparison.firstMismatch?.message ?? "V9 animation blending trace failed."}\n`);
  }
