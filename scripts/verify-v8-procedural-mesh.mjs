...
export async function verifyV8ProceduralMesh(options = {}) {
  const root = options.repoRoot ?? repoRoot;
  const run = options.run ?? runCommand;
  const artifactDir = options.artifactDir ?? resolve(root, "artifacts/v8/procedural-mesh");
...
async function main() {
  ...
  if (json) {
    ...
    process.stdout.write(`V8 procedural mesh gate passed. Report: ${result.reportPath}\n`);
  } else {
    const failed = result.steps.find((step) => step.exitCode !== 0);
    process.stderr.write(`V8 procedural mesh gate failed at '${failed?.name ?? "unknown"}'. Report: ${result.reportPath}\n`);
  }
