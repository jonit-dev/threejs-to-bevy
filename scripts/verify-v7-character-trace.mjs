  ...
  return {
    diagnostics:
      firstMismatch === undefined
        ...
        : [
            {
              code: "TN_VERIFY_V7_CHARACTER_TRACE_MISMATCH",
...
function mismatch(path, expected, actual) {
  return {
    actual,
    expected,
    message: `V7 character trace mismatch at ${path}: expected ${JSON.stringify(expected)} but received ${JSON.stringify(actual)}.`,
    path,
  };
}
...
function normalizeReport(report) {
  return {
    observations: (report.observations ?? []).map(sortObjectKeys).sort((left, right) => left.entity.localeCompare(right.entity)),
...
async function main() {
  const result = await verifyV7CharacterTrace({
    artifactDir: process.argv[3],
    bundlePath: process.argv[2],
  });
  if (result.ok) {
    process.stdout.write(`V7 character trace passed. Diff: ${result.artifacts.diffPath}\n`);
  } else {
    process.stderr.write(`${result.comparison.firstMismatch?.message ?? "V7 character trace failed."}\n`);
  }
