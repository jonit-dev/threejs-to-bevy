...
test("should build deterministic sqlite catalog from seed jsonl", async () => {
  ...
  try {
    ...
    const summary = query(first, "SELECT 'schema_version' AS key, value FROM catalog_meta WHERE key = 'schema_version' UNION ALL SELECT 'asset_count' AS key, COUNT(*) AS value FROM asset_files UNION ALL SELECT 'direct_count' AS key, COUNT(*) AS value FROM asset_files WHERE is_direct_download = 1 UNION ALL SELECT 'hdri_count' AS key, COUNT(*) AS value FROM asset_files WHERE file_role = 'hdri-index' UNION ALL SELECT 'material_count' AS key, COUNT(*) AS value FROM asset_files WHERE file_role = 'material-index' UNION ALL SELECT 'ambientcg_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'ambientcgAssetId' UNION ALL SELECT 'ambientcg_map_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'ambientcgMapKind' UNION ALL SELECT 'workflow_doc_hash' AS key, value FROM catalog_meta WHERE key = 'workflow_doc_sha256' UNION ALL SELECT 'ambientcg_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'ambientcg_snapshot_sha256' UNION ALL SELECT 'os3a_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'os3a_snapshot_sha256' UNION ALL SELECT 'polyhaven_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'polyhaven_snapshot_sha256';");
    const byKey = Object.fromEntries(summary.map((row) => [row.key, row.value]));
    assert.equal(byKey.schema_version, "1");
    assert.equal(Number(byKey.asset_count) > 100000, true);
