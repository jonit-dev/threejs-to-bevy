...
test("should build deterministic sqlite catalog from seed jsonl", async () => {
  ...
  try {
    ...
    const summary = query(first, "SELECT 'schema_version' AS key, value FROM catalog_meta WHERE key = 'schema_version' UNION ALL SELECT 'asset_count' AS key, COUNT(*) AS value FROM asset_files UNION ALL SELECT 'search_index_count' AS key, COUNT(*) AS value FROM asset_search UNION ALL SELECT 'search_doc_count' AS key, COUNT(*) AS value FROM asset_search_docs UNION ALL SELECT 'duplicate_download_urls' AS key, COUNT(*) AS value FROM (SELECT download_url FROM asset_files WHERE download_url IS NOT NULL AND download_url != '' GROUP BY download_url HAVING COUNT(*) > 1) UNION ALL SELECT 'direct_count' AS key, COUNT(*) AS value FROM asset_files WHERE is_direct_download = 1 UNION ALL SELECT 'direct_glb_count' AS key, COUNT(*) AS value FROM asset_files WHERE is_direct_download = 1 AND format = 'glb' UNION ALL SELECT 'model_count' AS key, COUNT(*) AS value FROM asset_files WHERE file_role IN ('model', 'model-index') UNION ALL SELECT 'hdri_count' AS key, COUNT(*) AS value FROM asset_files WHERE file_role = 'hdri-index' UNION ALL SELECT 'material_count' AS key, COUNT(*) AS value FROM asset_files WHERE file_role = 'material-index' UNION ALL SELECT 'ambientcg_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'ambientcgAssetId' UNION ALL SELECT 'ambientcg_compact_map_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'ambientcgMapKinds' UNION ALL SELECT 'ambientcg_map_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'ambientcgMapKind' UNION ALL SELECT 'objaverse_count' AS key, COUNT(*) AS value FROM asset_source_metadata WHERE key = 'objaverseUid' UNION ALL SELECT 'workflow_doc_hash' AS key, value FROM catalog_meta WHERE key = 'workflow_doc_sha256' UNION ALL SELECT 'ambientcg_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'ambientcg_snapshot_sha256' UNION ALL SELECT 'objaverse_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'objaverse_snapshot_sha256' UNION ALL SELECT 'os3a_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'os3a_snapshot_sha256' UNION ALL SELECT 'polyhaven_snapshot_hash' AS key, value FROM catalog_meta WHERE key = 'polyhaven_snapshot_sha256';");
    ...
    assert.equal(Number(byKey.asset_count) > (hasObjaverseSnapshot ? 150000 : 10000), true);
    assert.equal(Number(byKey.search_index_count), Number(byKey.asset_count));
    assert.equal(Number(byKey.search_doc_count), Number(byKey.asset_count));
    assert.equal(Number(byKey.duplicate_download_urls), 0);
    ...
    assert.equal(Number(byKey.material_count) > 9000, true);
    assert.equal(Number(byKey.ambientcg_count) > 10000, true);
    assert.equal(Number(byKey.ambientcg_compact_map_count) > 7000, true);
    assert.equal(Number(byKey.ambientcg_map_count), 0);
