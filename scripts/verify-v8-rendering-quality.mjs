...
export async function verifyV8RenderingQuality(options = {}) {
  ...
  const targets = resolveArtifactTargets({ gate: "rendering-quality", owner: { kind: "aggregate", name: "rendering-quality" }, root });

  const artifactDir = options.artifactDir ?? targets.absoluteDir;
  const reportPath = options.reportPath ?? resolve(artifactDir, "verification-report.json");
  const bundlePath = resolve(root, "packages/ir/fixtures/conformance/v5-drift-surface/game.bundle");
  const steps = [];
