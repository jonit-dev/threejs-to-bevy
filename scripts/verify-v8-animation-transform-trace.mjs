...
export async function verifyV8AnimationTransformTrace(options = {}) {
  ...
  const artifactDir = options.artifactDir ?? resolve(root, "artifacts/v8/animation-transform");
...
async function runWebTrace(root, bundlePath) {
  ...
  return normalizeReport({
    ...
    transformSamples: runtime.sampleTransformAnimations(bundle.animations, { timeSeconds: 0.5 }),
    version: "0.1.0",
  });
}
  ...
  return {
    diagnostics:
      firstMismatch === undefined
        ? []
        : [{ code: "TN_VERIFY_V8_TRANSFORM_ANIMATION_TRACE_MISMATCH", message: firstMismatch.message, path: firstMismatch.path, severity: "error" }],
    firstMismatch,
    status: firstMismatch === undefined ? "pass" : "fail",
    summary: {
      nativeSamples: native.transformSamples.length,
      webSamples: web.transformSamples.length,
    },
  };
}
...
function findFirstMismatch(web, native) {
  ...
  if (JSON.stringify(web.transformSamples) !== JSON.stringify(native.transformSamples)) {
    return mismatch("transformSamples", web.transformSamples, native.transformSamples);
  }
  return undefined;
}
...
function mismatch(path, expected, actual) {
  return {
    actual,
    expected,
    message: `V8 transform animation trace mismatch at ${path}: expected ${JSON.stringify(expected)} but received ${JSON.stringify(actual)}.`,
    path,
  };
}
...
function normalizeReport(report) {
  return {
    observations: (report.observations ?? []).map(sortObjectKeys),
    schema: report.schema,
    transformSamples: (report.transformSamples ?? []).map(sortObjectKeys).sort((left, right) => left.clip.localeCompare(right.clip) || left.target.localeCompare(right.target) || left.channel.localeCompare(right.channel)),
...
async function main() {
  const result = await verifyV8AnimationTransformTrace({
    artifactDir: process.argv[3],
    bundlePath: process.argv[2],
  });
  if (result.ok) {
    process.stdout.write(`V8 transform animation trace passed. Diff: ${result.artifacts.diffPath}\n`);
  } else {
    process.stderr.write(`${result.comparison.firstMismatch?.message ?? "V8 transform animation trace failed."}\n`);
  }
