...
export const RELEASE_FOCUSED_GATES: readonly FocusedGate[] = [
  ...
  { name: "verify v9 assets gltf scene workflow", reportPath: "tools/verify/artifacts/assets-gltf-scene-workflow/diff.json", script: "verify:v9:assets-gltf-scene-workflow" },
  ...
  { name: "verify particle commands", reportPath: "tools/verify/artifacts/particle-commands/verification-report.json", script: "verify:particle-commands" },
  ...
  { name: "verify template production", reportPath: "tools/verify/artifacts/template-production/verification-report.json", script: "verify:template-production" },
...
export async function runReleaseGate(options: ReleaseGateOptions = {}): Promise<ReleaseGateResult> {
  ...
  for (const [name, command, args] of [
    ...
    ["build authoring package", "pnpm", ["--filter", "@threenative/authoring", "build"]],
