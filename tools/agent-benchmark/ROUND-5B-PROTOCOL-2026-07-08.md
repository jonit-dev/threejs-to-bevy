...
- Per-run churn budgets are green for direct ThreeNative and typed-spec runs:
  engine-source search `0`, standalone verify `0`, artifact forensics `0`
  after the first artifact read, missing iterate `0`, missing discovery `0`,
  repeated file read `0`, failed command `0`, repeated assertion `0`, and
  repeated diagnostic `0`.
