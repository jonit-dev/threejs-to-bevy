...
| ⚠️ | Lights and shadows | Portable cascaded sun shadows share authored distance/split/blend/stabilization intent and requested/applied reports. Portable planar contact shadows map to adapter-private capture/blur/composite pipelines. Deterministic baked SH2 probes now light both adapters: web uses bounded camera-weighted SH, while Bevy 0.14 honestly reports a global SH-L0 ambient approximation. Tooling BVH and native parry `TriMesh` services also query rendered geometry independently of Collider metadata for baking and proof. Spherical/area and mixed-light baking remain diagnostic boundaries. | Ambient/directional/point/spot lights, stable cascaded and contact shadows, environment maps, deterministic baked probes, private scene-ray queries, and bounded quality profiles. | `pnpm verify:focused verify:shadow-cascade-stability`, `pnpm verify:focused verify:contact-shadows`, `pnpm verify:focused verify:baked-gi`, `pnpm verify:focused verify:feature-parity-visual-polish` |
...
| ⚠️ | Rendering and post-processing | Calibrated lighting/material/dense proof, fitted ACES output, monotonic AO, bloom, DOF, motion trails, bounded SSR, authored volumetrics, temporal web SSGI, and baked SH2 GI have focused evidence. Bevy 0.14 reports bounded homogeneous-fog, ambient/SSAO SSGI, and global SH-L0 probe approximations. Shared semantics remain incomplete for decals, arbitrary custom post, and GPU instance attributes. | Web/Bevy scene rendering, fog/sky/tone, bounded volumetrics, SSGI and baked GI, bloom, AO, DOF, motion blur, SSR, anti-aliasing, LOD, instancing, render-look profiles, and advanced renderer boundaries. | `pnpm verify:focused verify:baked-gi`, `pnpm verify:focused verify:ssgi`, `pnpm verify:focused verify:volumetrics`, `pnpm verify:focused verify:feature-parity-visual-polish`, `pnpm verify:rendering-photoreal` |
...
- [x] `P2` Bounded volumetric height fog and directional shadowed god rays,
      with paired region proof and an explicit Bevy 0.14 homogeneous-height
      approximation report; local volumes and non-directional shafts remain
      deferred
...
- [x] `P3` Bounded screen-space reflection intent with web/native wet-floor
      ...
      SSGI is promoted separately with a classified Bevy approximation
