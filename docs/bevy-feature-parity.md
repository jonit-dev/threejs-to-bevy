...
| ⚠️ | Rendering and post-processing | Calibrated lighting/material/dense proof, a shared ACES exposure anchor, intensity-1.0 bloom core/halo falloff, monotonic AO, DOF, motion trails, bounded SSR, authored volumetrics, temporal web SSGI, spatial native SSGI, and baked SH2 GI have focused evidence. The deterministic hero-interior gate now captures fresh authored output, bounds thin-surface SSR, aligns native SSGI/ambient and volumetric haze, and proves native/web luminance, contrast, highlights, warm mids, shadow range, and surface-detail energy. The showcase intentionally uses flat portable PBR surfaces on both adapters; web-only `StylizedNature` detail tiling is outside this claim. Bevy 0.14 deferred rendering retains the documented SH-L0 plus spatial-SSGI fallback and lacks temporal SSGI resolve. Shared semantics remain incomplete for decals, arbitrary custom post, and GPU instance attributes. | Web/Bevy scene rendering, fog/sky/tone, bounded volumetrics, SSGI and baked GI, bloom, AO, DOF, motion blur, SSR, anti-aliasing, LOD, instancing, render-look profiles, and advanced renderer boundaries. | `pnpm verify:focused verify:lighting-showcase`, `pnpm verify:focused verify:baked-gi`, `pnpm verify:focused verify:ssgi`, `pnpm verify:focused verify:volumetrics`, `pnpm verify:focused verify:feature-parity-visual-polish`, `pnpm verify:rendering-photoreal` |
...
- [x] `P3` Bounded screen-space reflection intent with web/native wet-floor
      ...
      SSGI is promoted separately with a bounded native spatial raymarch
