...
| Mesh primitives | ✅ | Box, sphere, and plane are proven. | Capsule/cylinder appear in some layers but are not broad cross-runtime parity claims. |
| Cameras | ⚠️ | Perspective camera and active camera path are usable; orthographic appears in IR/Bevy observations. | General camera resource model and full orthographic visual parity are not complete. |
...
| Physics/collision | ⚠️ | V6 box/sphere/capsule collider validation, rigid-body fields, deterministic collision/trigger event phases for fixed traces; V7 contract metadata now validates portable collider `layer`/`mask` filters and declares `physics.overlap` / `physics.shapeCast` service permissions. | Runtime shape casts, overlap observations, solver behavior, deterministic ordering, dynamic mesh colliders, and deeper physics parity are not claimed yet. |
