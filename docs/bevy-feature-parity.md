...
| ⚠️ | Physics and character movement | Web/Bevy now share lossless primitive body/collider mapping, configurable gravity, exact mass/capsules, collider-only statics, live contact phases, same-tick force/impulse services, character/sensor filters, and live hinge/slider/suspension constraints. Script queries remain conservative snapshot geometry rather than retained Rapier query-pipeline evidence; richer constraints, vehicles, soft bodies, ragdolls, arbitrary triangle narrow phase, and backend handles remain boundaries. | Fixed-tick physics, primitive bodies/colliders, contacts, bounded queries, character movement, mesh collider policy, promoted joints, and nav diagnostics. | `pnpm verify:focused verify:feature-parity-physics-native`, `pnpm verify:physics-self-verification`, `pnpm verify:character-physics-contacts` |
...
- [ ] Replace conservative script query snapshots and primitive proof traces
      with retained live Rapier query/observation evidence for every promoted
      shape and rotation case
