...
| Audio | ⚠️ | Local OGG/WAV validation, web HTML-audio sink, Bevy autoplay loop spawning, portable volume, deterministic audio command observations, and V7 bus/listener/spatial-emitter metadata with routed command reports through `v7-spatial-audio-buses`. | Stop/cleanup lifecycle traces, real spatial attenuation, mixer effects, streaming/network audio, and richer UI/audio services remain incomplete. |
