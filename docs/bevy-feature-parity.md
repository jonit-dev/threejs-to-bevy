...
- [ ] `P1` UI size constraints and scrolling
...
| UI | ⚠️ | Retained UI IR, validation, web DOM overlay, Bevy entity spawning, conformance UI tree, resource-bound bar, focusable button; V7 focus order, navigation links, input action refs, safe-area metadata, and fixed web/native focus/activation trace; explicit flex layout metadata now validates and maps direction, alignment, justification, gaps, padding, size, grow, overflow clipping, z-index layering, absolute positioning, and inset anchors across web DOM overlay and Bevy UI. | Size constraints, scrolling, richer styling, platform widgets, broad gamepad/touch coverage, and accessibility diagnostics remain incomplete. |
