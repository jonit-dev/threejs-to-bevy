...
| ⚠️ | Physics and character movement | Phases 1-4 add shared bounded compound colliders, same-tick force/impulse-at-point, retained Rapier queries, raycast-wheel/tire/surface simulation, production vehicle control, and pre-release lift/drag/stall/thrust/wind simulation. Paired traces prove exact child/wheel/contact identity, drivetrain outcomes, deterministic aerodynamic analytic cases, and numeric parity; hash-bound browser review and real web/graphical-desktop playtests complete the Phase 4 checkpoint. Richer constraints, destruction, soft bodies, ragdolls, dynamic triangle children, arbitrary triangle narrow phase, and backend handles remain boundaries. | Fixed-tick physics, primitive and bounded compound colliders, contacts, retained bounded queries, bounded wheel/tire/surface, drivetrain/controller, and aerodynamic simulation, character movement, mesh collider policy, promoted joints, and nav diagnostics. | `pnpm verify:physics-self-verification`; `pnpm verify:focused verify:advanced-physics-wheels`; `pnpm verify:focused verify:advanced-physics-drivetrain`; `pnpm verify:focused verify:advanced-physics-aerodynamics`; `tools/verify/artifacts/advanced-physics/phase-4-aerodynamics/verification-report.json` |
