...
| ⚠️ | Physics and character movement | Phases 1-3 add shared bounded compound colliders, same-tick force/impulse-at-point, retained Rapier queries, raycast-wheel/tire/surface simulation, and production vehicle control. Paired traces prove exact child/wheel/contact identity, authored order, full chassis/wheel observations, engine/gear/clutch and differential torque paths, steering/braking/reverse outcomes, ABS/TCS transitions, and deterministic retry; hash-bound browser review and real web/graphical-desktop playtests complete the Phase 3 checkpoint. Aerodynamics, richer constraints, destruction, soft bodies, ragdolls, dynamic triangle children, arbitrary triangle narrow phase, and backend handles remain boundaries. | Fixed-tick physics, primitive and bounded compound colliders, contacts, retained bounded queries, bounded wheel/tire/surface and drivetrain/controller simulation, character movement, mesh collider policy, promoted joints, and nav diagnostics. | `pnpm verify:physics-self-verification`; `pnpm verify:focused verify:advanced-physics-wheels`; `pnpm verify:focused verify:advanced-physics-drivetrain`; `tools/verify/artifacts/advanced-physics/phase-3-drivetrain/verification-report.json` |
