...
| ⚠️ | Physics and character movement | Phases 1-5 add shared bounded compound colliders, same-tick force/impulse-at-point, retained Rapier queries, raycast-wheel/tire/surface simulation, production vehicle control, pre-release aerodynamics, and pre-release rich motorized/breakable joints. Paired traces prove exact identities and ordering, drivetrain/aerodynamic outcomes, bounded joint loads, one-shot break/removal, and joint-only reconciliation without unrelated body churn; real web/graphical-desktop playtests cover the integrated Phase 4 checkpoint. Destruction, soft bodies, ragdolls, dynamic triangle children, arbitrary triangle narrow phase, backend handles, and final release enrollment remain boundaries. | Fixed-tick physics, primitive and bounded compound colliders, contacts, retained bounded queries, bounded wheel/tire/surface, drivetrain/controller, aerodynamic and rich-joint simulation, character movement, mesh collider policy, and nav diagnostics. | `pnpm verify:physics-self-verification`; `pnpm verify:focused verify:advanced-physics-wheels`; `pnpm verify:focused verify:advanced-physics-drivetrain`; `pnpm verify:focused verify:advanced-physics-aerodynamics`; `pnpm verify:focused verify:advanced-physics-joints`; `tools/verify/artifacts/advanced-physics/phase-5-joints/verification-report.json` |
