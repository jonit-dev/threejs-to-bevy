...
| Persistence/settings | ⚠️ | V9-06 carries schema-backed `local-data.ir.json` for declared resources/components, local settings, save-slot metadata, migration diagnostics, and autosave/checkpoint metadata across SDK, compiler, IR validation, web helpers, and Bevy loader/runtime diagnostics; V9-05 Phase 1 also promotes controls settings rows with retained UI node links and persisted logical binding overrides through web local storage helpers and Bevy local JSON reload/apply before the first input snapshot. Durable platform storage backends and cloud/account save integration remain unclaimed. | Broader import/export UX, cloud/account save integration, and richer settings-screen polish remain incomplete. |
