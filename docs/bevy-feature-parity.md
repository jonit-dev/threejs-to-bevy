...
| ⚠️ | Physics and character movement | Advanced compound bodies, wheel/vehicle simulation, aerodynamics, rich joints, deterministic pre-fracture, physical pieces, contact damage, regional activation, budgets, cleanup, descriptor-derived authoring, and normalized debug telemetry have paired implementation evidence. Three plan-first major-game forcing functions add five current matching web/desktop proof pairs, hash-bound manual review, and the exact 60-second dense workload under the release-enrolled gate. Soft bodies, fluids, runtime mesh cutting, ragdolls, dynamic triangle children, arbitrary triangle narrow phase, and backend handles remain boundaries. | Fixed-tick physics, primitive and bounded compound colliders, contacts, retained bounded queries, bounded wheel/tire/surface, drivetrain/controller, aerodynamic, rich-joint, and bounded pre-fractured destruction simulation, character movement, mesh collider policy, and nav diagnostics. | `pnpm verify:physics-self-verification`; `pnpm verify:focused verify:advanced-physics-wheels`; `pnpm verify:focused verify:advanced-physics-drivetrain`; `pnpm verify:focused verify:advanced-physics-aerodynamics`; `pnpm verify:focused verify:advanced-physics-joints`; `pnpm verify:focused verify:advanced-physics-destruction`; `pnpm verify:focused verify:advanced-physics-major-games` |
