...
- [x] `P2` Deterministic 3D mesh drag-picking trace for pointer-ray/mesh-pick workflows
- [ ] `P2` UI-target drag-and-drop picking events
...
- [x] `P1` Portable local-data contract for save-slot metadata over declared resources/components
- [x] `P1` Local settings key contract for controls, audio, video, and accessibility options
- [x] `P2` Save migration/version metadata and diagnostic hints in IR
- [x] `P2` Checkpoint hook metadata in IR
- [ ] `P1` Runtime save/load services and local web/native storage backends
- [ ] `P1` CLI import/export for local data
- [ ] `P2` Autosave/checkpoint execution semantics
...
- [x] Fixed listener-emitter attenuation trace for one spatial one-shot
- [ ] `P1` Real 3D spatial panning/output and listener movement
...
- [x] V8 support-evidence artifact gate for focused PRD artifacts and remaining gaps
...
| Assets/glTF/scenes | ⚠️ | Bundle-local glTF/GLB, `.bin`, and texture dependencies; V3 environment instances resolve to real model scenes in web and Bevy; V8-10 now has a narrow web/native `threenative.asset-load-sync-trace` that proves deterministic bundle-local path-asset load ordering, a `bundle.requiredAssets` ready barrier, and environment glTF source-asset to instance/LOD inspection data under `artifacts/conformance/v8-asset-load-gltf-inspection/`. | Public asset-group declarations, spawned glTF node query/update handles, material override handles, editor inspection commands, watch diagnostics, state-preserving hot reload, and some asset diagnostics remain incomplete. |
...
| Diagnostics | ⚠️ | Stable IR/compiler/CLI/native diagnostic shapes, severity, suggestions, metadata preservation, V5/V6 ranges; `pnpm verify:v8:support-evidence` now writes `artifacts/v8/support-evidence/verification-report.json` with V8 docs-check command output, focused V8 PRD verifier output where wired, required report/screenshot artifact presence checks, and explicit remaining gaps per tracked V8 PRD. | Some asset/runtime failures still need better domain-specific codes and target-specific repair hints; the V8 support-evidence report is infrastructure only and does not prove V8-18 inspector, debug overlay, package repair, or profiler completion. |
