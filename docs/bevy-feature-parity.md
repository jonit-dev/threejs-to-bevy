...
| ⚠️ | Transforms, math, and geometry | Promoted geometry has shared contract plus web/Bevy coverage. Deterministic CSG/boolean solving lowers through compile-time `MeshBuilder` to static custom meshes, and generated scene-mesh LOD now emits deterministic `.lod.N` variants with actual geometry/handle swaps at shared thresholds. Registry-derived bundle, collider, trace, and rendered evidence covers both; runtime CSG, advanced runtime deformation/generation, environment HLOD/fades/impostors, streaming, and storage-buffer geometry remain boundaries. | Stable transforms, primitives, procedural meshes, compile-time CSG, generated scene-mesh LOD, bounds, paths, terrain, and geometry diagnostics. | `pnpm verify:conformance`, `pnpm verify:v8-procedural-mesh -- --json`, `pnpm verify:focused verify:generated-mesh-lod`, source/IR/compiler/runtime tests |
...
| ⚠️ | Lights and shadows | Portable cascaded sun shadows share authored distance/split/blend/stabilization intent and requested/applied reports. Portable planar contact shadows map to adapter-private capture/blur/composite pipelines. Deterministic baked SH2 probes now light both adapters: web uses bounded camera-weighted SH and native forward rendering uses bounded Bevy ambient-cube irradiance volumes. Bevy 0.14 deferred lighting has an upstream irradiance shader-definition defect, so SSR/deferred scenes honestly report an SH-L0 atmosphere baseline composed with native spatial SSGI. SSGI-enabled native scenes add shadowless ceiling- and floor-bounce fills as bounded adapter approximations for the missing hemisphere lobes. The composed hero interior holds native/web exposure and contrast in the same measured range. Tooling BVH and native parry `TriMesh` services also query rendered geometry independently of Collider metadata for baking and proof. Spherical/area and mixed-light baking remain diagnostic boundaries. | Ambient/directional/point/spot lights, stable cascaded and contact shadows, environment maps, deterministic baked probes, private scene-ray queries, and bounded quality profiles. | `pnpm verify:focused verify:shadow-cascade-stability`, `pnpm verify:focused verify:contact-shadows`, `pnpm verify:focused verify:baked-gi`, `pnpm verify:focused verify:lighting-showcase`, `pnpm verify:focused verify:feature-parity-visual-polish` |
...
| ⚠️ | Rendering and post-processing | Calibrated lighting/material/dense proof, a shared ACES exposure anchor, intensity-1.0 bloom core/halo falloff, monotonic AO, DOF, motion trails, bounded SSR with thin-surface depth calibration, authored volumetrics, temporal web SSGI, bounded/jittered spatial native SSGI with documented directional ceiling/floor-bounce approximations, and baked SH2 GI have focused evidence. The deterministic hero-interior gate now captures fresh authored output, bounds thin-surface SSR, aligns native SSGI/ambient and volumetric haze, and proves native/web luminance, contrast, highlights, warm mids, shadow range, ceiling/right-room regions, and surface-detail energy. The showcase intentionally uses flat portable PBR surfaces on both adapters; web-only `StylizedNature` detail tiling is outside this claim. Bevy 0.14 deferred rendering retains the documented SH-L0 plus spatial-SSGI fallback and lacks temporal SSGI resolve; SSR-selected smooth native wet patches use a documented reflectance-suppression approximation when that deferred boundary removes the web reflection. Shared semantics remain incomplete for decals, arbitrary custom post, and GPU instance attributes. | Web/Bevy scene rendering, fog/sky/tone, bounded volumetrics, SSGI and baked GI, bloom, AO, DOF, motion blur, SSR, anti-aliasing, LOD, instancing, render-look profiles, and advanced renderer boundaries. | `pnpm verify:focused verify:lighting-showcase`, `pnpm verify:focused verify:baked-gi`, `pnpm verify:focused verify:ssgi`, `pnpm verify:focused verify:volumetrics`, `pnpm verify:focused verify:feature-parity-visual-polish`, `pnpm verify:rendering-photoreal` |
...
| ⚠️ | Physics and character movement | Phases 1-7 have paired implementation evidence for compound bodies, wheel/vehicle simulation, aerodynamics, rich joints, deterministic pre-fracture, physical pieces, contact damage, regional activation, budgets, cleanup, descriptor-derived authoring, and toggleable normalized debug telemetry. Phase 8 web objective evidence is passing, but current-source desktop objective proof, the exact real 60-second workload, production review, and independent checkpoint remain open before major-games release enrollment. Soft bodies, fluids, runtime mesh cutting, ragdolls, dynamic triangle children, arbitrary triangle narrow phase, and backend handles remain boundaries. | Fixed-tick physics, primitive and bounded compound colliders, contacts, retained bounded queries, bounded wheel/tire/surface, drivetrain/controller, aerodynamic, rich-joint, and bounded pre-fractured destruction simulation, character movement, mesh collider policy, and nav diagnostics. | `pnpm verify:physics-self-verification`; `pnpm verify:focused verify:advanced-physics-wheels`; `pnpm verify:focused verify:advanced-physics-drivetrain`; `pnpm verify:focused verify:advanced-physics-aerodynamics`; `pnpm verify:focused verify:advanced-physics-joints`; `pnpm verify:focused verify:advanced-physics-destruction`; pending `pnpm verify:focused verify:advanced-physics-major-games` |
...
| ✅ | Persistence and settings | Declared save/settings data now uses exact bounded records on both adapters. Native QuickJS no longer owns durability: Rust stages, syncs, and atomically renames app/target-profile records, executes Local Data 0.2 transforms, and restores progress plus settings in a second reference process. Cloud/account and arbitrary script filesystem access remain deferred boundaries; a user-driven packaged-game relaunch was not separately captured. | Save slots, local settings, executable migration transforms, checkpoint/autosave, durable Bevy backend, and fail-closed corruption/forward/interrupted controls. | `pnpm verify:focused verify:persistence-reload`; `runtime-bevy/crates/threenative_runtime/tests/persistence_storage.rs` |
...
- [x] `P1` Compile-time CSG and boolean mesh authoring lowered to static custom meshes, with web/native visual and derived-collider trace evidence; runtime CSG remains a `P3` diagnostic boundary
...
- [x] `P1` Portable planar `ContactShadows` component with matched bounded
      size, height, resolution, softness, opacity, and static/dynamic update
      fields; web and Bevy use adapter-private capture, separable-blur, and
      composite resources without mutating authored materials. Catalog-owned
      `pnpm verify:focused verify:contact-shadows` checks portable reports,
      monotonic localized darkening, static steady-state cost, and paired
      web/native captures.
...
- [x] `P2` Bounded volumetric height fog and directional shadowed god rays,
      with paired region proof, independently calibrated shaft radiance, and a
      native analytic height-fog post pass preserving authored base height and
      falloff; local volumes and non-directional shafts remain deferred
...
- [x] `P3` Bounded screen-space reflection intent with web/native wet-floor
      ...
      mirrors and broader material/reflection semantics remain diagnostic;
      SSGI is promoted separately with a bounded native spatial neighborhood
      gather and no temporal resolve
...
- [x] `P1` Environment/model source-asset LOD observation coverage (generated
      scene-mesh swapping is promoted separately below)
- [x] `P1` Generated scene-mesh LOD with deterministic compiler variants,
      exact shared thresholds, actual web geometry/native `Handle<Mesh>` swaps,
      and paired `verify:generated-mesh-lod` rendered evidence
...
- [x] Configurable runtime gravity, exact authored primitive mass/inverse mass,
      collider-only implicit statics, corrected total-height capsules, and
      native live enter/stay/exit contact queues
- [x] Same-tick declared force, torque, impulse, angular impulse, and velocity
      body services for fixed-update scripts across web and Bevy QuickJS
- [x] Phase 1 bounded compound colliders and same-tick force/impulse-at-point,
      with stable child IDs/local poses and paired causal-negative traces
- [x] Live Rapier hinge, slider, and suspension constraints with portable
      anchors, axes, limits/travel, stiffness, damping, and target validation
- [x] Phase 1 retained Rapier raycast, shape-cast, and overlap observations,
      including exact compound-child identity in paired web/native evidence
- [x] Phase 2 bounded wheel assemblies, tire curves, and authored asphalt/ice
      surfaces with exact contact semantics, registry-toleranced full chassis
      and wheel/contact observations, consumed chassis-child visual targets with
      bounded spin interpolation, steering/braking outcomes, drive/steer/brake
      flag causal controls, and hash-bound continuous-crossing evidence
- [ ] Extend retained query/contact evidence across every promoted rotated shape
      and normalized contact-manifold field
...
- [x] `P1` Optional desktop CEF overlays receive retained game snapshots through the same subscribe contract as web overlays, including replay and bounded payload validation (`cargo test -p threenative_runtime --test overlay_cef --features native-overlay-cef`)
- [x] `P1` Partial/diagnostic: Linux desktop React overlays use CEF off-screen
...
- [x] `P1` Promoted: the shared window-policy diagnostic set and paired
      resize/scale report are drift-checked by
...
- [x] `P2` Generated tone command/support metadata is web/native trace-proved;
      synthesized native waveform output remains outside this claim.
