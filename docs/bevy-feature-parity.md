...
| ------ | ---- | -------- | -------------------- | ---------------- |
| ✅ | ECS, app, and scheduling | No active adapter gap; shared contract stays guarded by conformance. | Portable ECS declarations, schedules, lifecycle, systems, resources, events, tags, groups, and runtime host semantics. | `pnpm verify:runtime-gameplay-host`, `pnpm verify:conformance` |
| ⚠️ | Transforms, math, and geometry | Shared contract plus both adapters for advanced geometry/deformation. | Stable transforms, primitives, procedural meshes, bounds, paths, terrain, and geometry diagnostics. | `pnpm verify:rendering-residuals`, source/IR/compiler tests |
| ✅ | Cameras and views | No active adapter gap for promoted camera behavior. | Perspective/orthographic cameras, active camera selection, split views, render targets, camera helpers, and screenshot/export workflows. | `tn scene proof-camera`, `tn playtest --follow`, conformance gates |
| ⚠️ | Lights and shadows | Both adapters; visual calibration and advanced light transport remain bounded. | Ambient/directional/point/spot lights, shadows, probes, environment maps, and bounded quality profiles. | `pnpm verify:render-look`, `pnpm verify:v10:visual-calibration` |
| ⚠️ | Materials, textures, and shaders | Both adapters plus shared asset/material import policy. | PBR fields, texture slots, alpha/emissive/specular controls, portable shader material v1, and advanced shader diagnostics. | `pnpm verify:portable-shader-material`, material/runtime tests |
| ⚠️ | Rendering and post-processing | Both adapters; visual parity is the main gap, not only Bevy. | Web/Bevy scene rendering, fog/sky/tone, bloom, anti-aliasing, LOD, instancing, render-look profiles, and advanced renderer boundaries. | `pnpm verify:render-look`, `pnpm verify:rendering-photoreal` |
| ✅ | Assets, glTF, and scenes | No active adapter gap for promoted asset loading; custom loaders stay a shared boundary. | Bundle-local assets, glTF dependency handling, asset catalogs, inspection, hot reload, streaming policy, and custom-loader diagnostics. | `tn asset inspect`, `pnpm verify:gltf-fidelity` |
| ⚠️ | Animation and particles | Both adapters for advanced rigging/VFX proof; raw backend animation remains shared boundary. | Clip metadata, playback, events, bounded graph data, masks, morph targets, and deterministic lightweight VFX. | `pnpm verify:animation-physics-residuals`, conformance fixtures |
| ⚠️ | Physics and character movement | Mostly Bevy/native proof depth, with shared solver semantics kept portable. | Fixed-tick physics, primitive bodies/colliders, contacts, queries, character movement, mesh collider policy, joints, and nav diagnostics. | `pnpm verify:physics-self-verification`, `pnpm verify:character-physics-contacts` |
| ✅ | Input, picking, and controls | No active adapter gap for promoted input/picking behavior. | Keyboard, mouse, pointer lock, gamepad, touch, picking, UI action dispatch, rebinding, and device diagnostics. | `pnpm verify:input-ui-polish`, conformance fixtures |
| ⚠️ | UI, text, and accessibility | Mostly Bevy/native for pixel-perfect UI, text, IME, and platform accessibility. | Retained UI, layout/style/text/buttons, focus, navigation, recipes, accessibility diagnostics, and unsupported native/widget boundaries. | `pnpm verify:input-ui-polish`, `pnpm verify:conformance` |
| ⚠️ | Window and platform runtime | Mostly Bevy/native and platform policy. | Window metadata, target profiles, resize observations, cursor/power diagnostics, and single-window policy. | Runtime config tests, target-profile fixtures |
| ✅ | Persistence and settings | No active adapter gap for declared save/settings behavior. | Save slots, local settings, migration metadata, checkpoint/autosave, durable Bevy backend, and cloud boundary diagnostics. | `pnpm verify:persistence-reload` |
| ⚠️ | Audio | Mostly Bevy/native and platform audio depth. | Local audio assets, playback commands, spatial/listener metadata, mixer/effect reports, routing diagnostics, and decoder/streaming boundaries. | `pnpm verify:production-hardening`, conformance fixtures |
| ✅ | Diagnostics, tooling, packaging, and performance | No active adapter gap for release-gated diagnostics; metrics vary by target. | Stable diagnostics, release gates, reports, budgets, profiler/debug evidence, package preflight, and repair hints. | `pnpm verify:release`, `pnpm verify:production-hardening` |
| ⚠️ | Editor and developer tools | Product boundary: browser/CLI editor is promoted; native desktop editor shell is deferred. | Source-backed editor operations, inspector mappings, preview/build proof, gizmos/debug tools, hot reload, and native editor boundary. | `pnpm verify:focused verify:editor-package`, editor operation tests |
| ⏭️ | Deferred or non-portable | Intentional product boundary, not a Three.js or Bevy parity gap. | Raw Bevy/Three.js authoring, backend handles, networking, 2D workflows, arbitrary platform APIs, and non-IR backend features. | Stable unsupported diagnostics |
