...
| ⚠️ | Transforms, math, and geometry | Promoted geometry has shared contract plus web/Bevy coverage. Deterministic CSG/boolean solving is promoted as compile-time `MeshBuilder` lowering to existing static custom-mesh assets, backed by registry-derived CSG/noise bundle, visual, and collider traces; runtime CSG, advanced runtime deformation, generated-mesh LOD selection, and storage-buffer geometry remain boundaries. | Stable transforms, primitives, procedural meshes, compile-time CSG, bounds, paths, terrain, and geometry diagnostics. | `pnpm verify:conformance`, `pnpm verify:v8-procedural-mesh -- --json`, source/IR/compiler tests |
