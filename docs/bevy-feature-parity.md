...
- Runtime performance proof now has a versioned sidecar contract, verifier,
  ...
  benchmark sidecar against target-profile budgets and density floors.
  `tn performance proof --target desktop` emits Bevy/native sidecars with
  measured static bundle counters and stable unsupported diagnostics for
  unpromoted native frame/draw/visibility counters; native metric promotion
  evidence remains unclaimed until those counters are measured.
...
- Project type generation, `defineBehavior` system metadata, and
  `tn actor add/update/list` archetypes improve the authoring/compiler path for
...
| Texture compression and delivery | Bevy's image loader is feature-gated for formats such as WebP, JPEG, KTX2/DDS/Basis paths, and device compressed formats. The runtime currently enables only a subset. | Smaller downloads, faster load, HDR/environment-map feasibility, and mobile budget control. | Source accepts target-specific variants only with baseline fallback diagnostics, and efficient-scale now budgets selected texture-variant bytes. Keep native compressed texture loading unclaimed until per-target runtime evidence lands. |
