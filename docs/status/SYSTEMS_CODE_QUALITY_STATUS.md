...
| 🟡 | Authoring operations and mutation surfaces | Operation truth is still split across authoring, CLI, MCP, editor, and smoke surfaces, but registry drift now fails through explicit adapter-surface coverage gates. | Execute PRD-004 executable operation descriptors so adapter metadata can be derived instead of allowlisted. | `docs/PRDs/done/other/adapter-surface-remediation-2026-07-08/PRD-002-adapter-surface-drift-gates.md`; `docs/PRDs/other/adapter-surface-remediation-2026-07-08/PRD-004-executable-authoring-operation-descriptors.md`; `tools/verify/src/adapterSurfaceDrift.test.ts`; `pnpm --filter @threenative/authoring test`; `pnpm verify:editor-required-operations` |
| 🟡 | CLI command surface | `tn` command metadata, help, and migrated dispatch now route through an incremental typed command registry with explicit legacy compatibility for unmigrated families. Some large command families still own local argv parsing until PRD-004/005 continue descriptor-backed migration. | Execute PRD-004 executable operation descriptors and continue migrating large source-document families from the registry substrate. | `docs/PRDs/done/other/adapter-surface-remediation-2026-07-08/PRD-003-cli-command-registry-and-shared-arg-plumbing.md`; `packages/cli/src/commands/registry.ts`; `packages/cli/src/index.ts`; `packages/cli/src/index.test.ts`; `pnpm --filter @threenative/cli test`; manual smoke: `tn actor list --json`, `tn build --project templates/structured-source-starter --json`, `tn proof diff --from /tmp/tn-proof-a.json --to /tmp/tn-proof-b.json --json` |
| 🟡 | Editor source operations | Editor operation payload builders, modal actions, coverage matrices, and smoke requirements still need descriptor-backed recipes, but enabled operation names and payload keys now fail against the authoring registry. | Execute PRD-005 editor operation metadata and composite recipes. | `docs/PRDs/done/other/adapter-surface-remediation-2026-07-08/PRD-002-adapter-surface-drift-gates.md`; `docs/PRDs/other/adapter-surface-remediation-2026-07-08/PRD-005-editor-operation-metadata-and-composite-recipes.md`; `packages/editor/src/adapters/editorModel.test.ts`; `packages/editor/src/state/editorStore.ts`; `pnpm --filter @threenative/editor test`; `pnpm verify:editor-required-operations` |
