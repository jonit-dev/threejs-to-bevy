...
| 🟡 | Rendering, materials, lights, cameras, and post-processing | Live reconciliation, cubemaps, visibility, stable cascaded shadows, portable contact shadows, layers, and teardown are implemented. Cascade controllers and the new cross-adapter contact-shadow pipelines own lifecycle-sensitive cameras, render targets, proxy meshes, materials, textures, and pass ordering; Bevy additionally needs private high-numbered render layers and bounded warmup before static capture cameras deactivate. Requested/applied cascade approximation reporting and contact-shadow cost/resource reports are therefore part of the contract. Web and native also share fitted ACES/exposure/saturation semantics, temporal frame accumulation, and depth-aware SSR intent. | Keep cascade texel stability, material-hook restoration, contact-shadow reconciliation/isolation/static-cost/cleanup tests, shared profile reporting, monotonic cross-adapter grounding metrics, specular-F0 conversion, AO monotonicity, aligned transform traces, exterior-trail asymmetry, and render-resource lifecycle tests green. | `packages/runtime-web-three/src/rendering/directionalShadowController.test.ts`; `packages/runtime-web-three/src/rendering/contactShadows.test.ts`; `tools/verify/src/shadowCascadeStability.ts`; `tools/verify/src/contactShadowsGate.ts`; `runtime-bevy/crates/threenative_runtime/tests/contact_shadows.rs`; `tools/verify/src/renderingPhotoreal.ts` |
