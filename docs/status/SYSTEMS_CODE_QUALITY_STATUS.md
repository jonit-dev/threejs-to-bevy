...
| 🟡 | Rendering, materials, lights, cameras, and post-processing | Live reconciliation, cubemaps, visibility, stable cascaded shadows, layers, and teardown are implemented. The web cascade controller adds per-material shader composition and multiple adapter-owned shadow resources whose camera ordering, hook restoration, and disposal are lifecycle-sensitive. Bevy 0.14 can preserve an authored first split and maximum distance but exponentially spaces remaining bounds, so requested/applied approximation reporting is part of the contract. Web and native also share fitted ACES/exposure/saturation semantics, temporal frame accumulation, and depth-aware SSR intent; persistent temporal and cascade resources remain bounded by focused cleanup and ordering tests. | Keep cascade texel-stability, material-hook restoration, multi-camera ordering, shared profile reporting, specular-F0 conversion, AO monotonicity, aligned transform traces, exterior-trail asymmetry, and render-resource lifecycle tests green; do not promote new split schemes without exact adapter evidence or explicit approximation semantics. | `packages/runtime-web-three/src/rendering/directionalShadowController.test.ts`; `tools/verify/src/shadowCascadeStability.ts`; `packages/runtime-web-three/src/render.test.ts`; `tools/verify/src/renderingPhotoreal.ts`; `runtime-bevy/crates/threenative_runtime/tests/rendering_atmosphere.rs` |
