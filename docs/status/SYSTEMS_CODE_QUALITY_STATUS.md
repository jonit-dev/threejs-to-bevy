...
| 🟡 | Rendering, materials, lights, cameras, and post-processing | Live reconciliation, cubemaps, visibility, stable cascades, contact shadows, bounded volumetrics, temporal web SSGI, deterministic baked SH2 probes, collider-independent tooling/native scene-ray queries, layers, and teardown are implemented. These pipelines own lifecycle-sensitive cameras, continuous-update policy, history buffers, shadow maps, render targets, materials, textures, query meshes, and pass ordering. Bevy uses reported bounded approximations for height falloff, SSGI color bleed, and baked-probe spatial variation. Web and native now share a measured ACES/exposure/saturation/contrast/bloom fit, and the complete photoreal matrix is green. Temporal invalidation, volumetric/SSGI shader compilation, probe hash coverage, scene-query rebuild policy for live topology changes, cascade sampling, and depth-aware SSR remain adapter-private drift risks. | Keep cascade stability, contact-shadow lifecycle/cost, volumetric proof, SSGI scheduling/history/orbit proof, baked-probe determinism/staleness/warm-lift proof, render-geometry query tests, shared profile reporting, the full photoreal color matrix, grounding metrics, AO monotonicity, aligned motion traces, and render-resource teardown green. Recalibrate the shared mappings when renderer or tone-mapping upgrades materially change output. | `packages/compiler/src/bake/sceneRayQuery.test.ts`; `runtime-bevy/crates/threenative_runtime/src/scene_ray_query.rs`; `packages/runtime-web-three/src/rendering/directionalShadowController.test.ts`; `packages/runtime-web-three/src/rendering/contactShadows.test.ts`; `packages/runtime-web-three/src/rendering/godrays/GodRaysPass.test.ts`; `packages/runtime-web-three/src/rendering/ssgi/temporalResolvePass.test.ts`; `packages/runtime-web-three/src/rendering/bakedProbeLighting.test.ts`; `tools/verify/src/volumetricsGate.ts`; `tools/verify/src/ssgiGate.ts`; `tools/verify/src/bakedGiGate.ts`; `runtime-bevy/crates/threenative_runtime/tests/rendering_atmosphere.rs`; `runtime-bevy/crates/threenative_runtime/tests/rendering.rs`; `tools/verify/src/renderingPhotoreal.ts` |
