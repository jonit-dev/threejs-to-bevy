...
| ✅ P1 | Environment skybox/map/terrain/source metadata commands | Resolved for environment document creation plus promoted skybox, environment-map, terrain, path, walkability, and source-asset LOD fields; light probes remain inspect-only. | `tn environment create`, `tn environment set-skybox`, `tn environment set-map`, `tn environment set-terrain`, `tn environment set-path`, `tn environment set-walkability`, `tn environment set-source-asset-lod` |
