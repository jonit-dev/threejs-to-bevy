...
| UI | Active | Retained structured UI is the portable path; optional React overlays can target web and Linux desktop through typed contracts, capability-checked native launch, input/visibility controls, snapshot delivery, and CEF off-screen Bevy textures. NVIDIA/Xwayland chooser, hover, Black-side start, 300 modal transitions with settled memory, multi-overlay lifecycle/z-order, resize and minimize/restore, pointer/wheel/key/focus routing, modal game-input suppression, and mounted AppImage packaging are proved; full IME, other compositors, Windows/macOS, platform screen readers, native style effects, and rendered world attachment remain boundaries. | [ui](status/capabilities/ui.md) |
| Assets | Active | Bundle-local assets include provider provenance, secret-isolated experimental model-provider jobs, and explicit-consent Linux x64 Blender recipes with checked real prop and humanoid animation proof; other Blender hosts and credential-backed provider promotion remain explicit gaps. | [assets](status/capabilities/assets.md) |
...
| Distribution | In progress | Versioned distribution source/IR, registry-derived planning, web static/ZIP/PWA, Linux x86-64 Bevy/embedded-webview, and Android x86-64/arm64 embedded-webview packaging are implemented. Android emulator resize and cold-relaunch persistence now pass; audio lifecycle and physical arm64 execution still fail closed, while promotion, other native hosts/runtimes, store credentials, and devices remain gated. | [distribution](status/capabilities/distribution.md) |
