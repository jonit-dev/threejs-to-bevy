...
| UI | Active | Retained structured UI is the portable path; optional React webview overlays have typed contracts, capability-checked native launch, proof-harness failure for unavailable desktop hosts, runtime input/visibility controls, and web/desktop snapshot delivery, while IME, virtual keyboards, platform screen readers, native style effects, and rendered world attachment remain truth-graded boundaries. | [ui](status/capabilities/ui.md) |
...
| Audio/platform | Active | Web/native bundle-local audio supports authoring-time generated SFX with mock provider evidence; runtime network audio remains unsupported. | [audio/platform](status/capabilities/audio-platform.md) |
...
| Game production | Active | Generated instructions prefer bounded SFX generation after a successful provider probe, retain offline fallbacks, and release-scan provider secrets; evidence is mock-only. | [game production](status/capabilities/game-production.md) |
