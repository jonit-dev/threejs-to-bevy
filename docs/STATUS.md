...
| UI | Active | Retained structured UI is the portable path; one optional React overlay bundle can target web and Linux desktop through typed contracts, capability-checked native launch, input/visibility controls, snapshot delivery, and a CEF off-screen Bevy texture. NVIDIA/Xwayland chooser, hover, Black-side start, repeated modal pixels, pointer/wheel/key/focus routing, modal game-input suppression, and mounted AppImage packaging are proved; multi-overlay lifecycle, full IME, memory-growth closure, other compositors, Windows/macOS, platform screen readers, native style effects, and rendered world attachment remain boundaries. | [ui](status/capabilities/ui.md) |
