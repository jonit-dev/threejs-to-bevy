...
| Physics | Active | Phases 1-2 promote compound colliders, at-point forces, retained queries, and wheel/tire/surface simulation; Phases 3-5 implement pre-release drivetrain, aerodynamics, and rich joints with paired evidence; Phase 6 has independently sourced paired destruction/debug proof; and Phase 7 adds descriptor-backed atomic authoring, field-level adapter drift guards, and normalized bounded telemetry while manual reviews, performance, and release enrollment remain active. | [physics](status/capabilities/physics.md) |
...
| Assets | Active | Bundle-local assets include provider provenance, relocatable material-aware model-test projects with fail-closed web runtime proof, secret-isolated experimental model-provider jobs, explicit-consent Linux x64 Blender recipes, and a reviewed bounded img2threejs Group subset with descriptor-derived CLI/MCP generation, durable reruns, accepted-hash ownership checks, embedded local/canvas textures, typed runtime extras, fixed source/reload visual proof, isolated network-blocked Chromium export, atomic validation/registration, and a focused deterministic gate with four-angle web plus assets-ready native load evidence; native pixel parity, broader material/upstream factories, other Blender hosts, and credential-backed provider promotion remain explicit gaps. | [assets](status/capabilities/assets.md) |
