...
| --- | --- | --- | --- |
| Authoring | Active | Structured source now has descriptor-derived atomic multi-file batches with plan hashes, generator ownership, crash recovery, bounded staging/diffs, typed client operations, and a 64 KiB-to-50 MiB regression gate, alongside transactional recipes, concrete game plans, compact constructs, cookbook/MCP discovery, playtest scaffolding, semantic proof gates, and the experimental typed game-spec slice. | [authoring](status/capabilities/authoring.md) |
...
| UI | Active | Retained structured UI now has registry-owned evidence tiers, paired responsive captures at two viewports, live web/native widget-state and action proof, and normalized ARIA/AccessKit metadata; nested/horizontal scroll, spatial fallback, focus narration, full IME, platform screen readers, native style effects, and rendered world attachment remain explicit partial or unsupported boundaries. Optional React overlays retain the proved Linux desktop CEF path. | [ui](status/capabilities/ui.md) |
