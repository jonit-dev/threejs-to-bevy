...
| Physics | Active | Advanced compound, wheel/drivetrain, aerodynamic, rich-joint, and bounded destruction contracts now have paired retained-Rapier evidence; descriptor-backed authoring and normalized debug telemetry are promoted, and three plan-first major-game forcing functions pass five current web/desktop proof pairs plus the exact dense 60-second release gate; aerodynamic validation now reports conservative spawn lift/weight, thrust/drag, damping, and trim viability before build, while the corrected angle-of-attack convention is regression-tested on both adapters. Soft bodies, fluids, runtime mesh cutting, ragdolls, and backend handles remain boundaries. | [physics](status/capabilities/physics.md) |
| UI | Active | Retained structured UI now has registry-owned evidence tiers, paired responsive captures at two viewports, causal idle/hover/selected and isolated shadow/gradient pixel proof, live web/native widget-state and action proof, bounded native visual effects, traced explicit bold-face selection, and normalized ARIA/AccessKit metadata; nested/horizontal scroll, spatial fallback, focus narration, full IME, platform screen readers, and rendered world attachment remain explicit partial or unsupported boundaries. Optional React overlays retain the proved Linux desktop CEF path, and pointer-mode web overlay frames forward keyboard events to the game window. | [ui](status/capabilities/ui.md) |
| Assets | Active | Bundle-local assets include provider provenance, relocatable material-aware model-test projects with fail-closed web runtime proof, secret-isolated experimental model-provider jobs, explicit-consent Linux x64 Blender primitive recipes with contained base-color/normal textures and bounded UV tiling plus bounded project-local source-GLB transform animation, permanent root-axis offsets, global collapse decimation, authored hinge pivots, and polygon-preserving split-by-axis targets, and a reviewed bounded img2threejs Group subset with descriptor-derived CLI/MCP generation, durable reruns, accepted-hash ownership checks, embedded local/canvas textures, typed runtime extras, fixed source/reload visual proof, isolated network-blocked Chromium export, atomic validation/registration, and a focused deterministic gate with four-angle web plus assets-ready native load evidence; native pixel parity, rigs, broader material/upstream factories, other Blender hosts, and credential-backed provider promotion remain explicit gaps. | [assets](status/capabilities/assets.md) |
| Audio/platform | Active | Web/native bundle-local startup, event, and script playback execute in their game loops with native entity-spawn proof; native sink controls execute except seek. A release-enrolled project audit now derives sound bindings and playback gain from durable source, measures source/effective intensity, reconciles generated loop intent, and rejects short, edge-imbalanced, or discontinuous loops; generated tones, device routing, and runtime network audio retain explicit boundaries. | [audio/platform](status/capabilities/audio-platform.md) |
| Native parity | Frozen for promotion | Bevy consumes emitted IR; the native playtest P0 is closed, headless desktop playtests now return a structured waived-headless warning instead of a winit crash (offscreen rendering remains unsupported), bounded gameplay parity pairs one humanoid smoke playtest plus a full-profile ball-push row across web/desktop, the closed Interaction subset has paired web/native trace and state conformance, trace/source-only rows are not broad promotions, and new native promotions require shipped-game evidence, native proof, and the native path decision; desktop playtests reuse prebuilt runtime binaries with libcef library-path probing and prove animation playback via readiness animation samples. | [native parity](status/capabilities/native-parity.md) |
| Game production | Active | Generated instructions prefer bounded SFX generation after a successful provider probe, retain offline fallbacks, and release-scan provider secrets; evidence is mock-only. PlacementSet and bounded cross-adapter Interaction recipes are documented with exact-match and conformance boundaries. Score/QA honor recorded `threenative.game-scope-blockers` waivers, and scale QA accepts aircraft/watercraft vehicle heroes. | [game production](status/capabilities/game-production.md) |
...
| Tooling/proof | Active | Verification owns current-run acceptance coverage, registry-backed bounded command summaries, focused-loop/template guidance gates, prompt-hash preparation, real Three.js/WebGL compliance, web/native Chrome/Perfetto performance traces, token/command caps, parity, and retained aggregate artifacts; generated-game QA now requires exact plan-owned duration proof plus focused-overlay and deterministic cross-target scenario lanes where applicable. The July 15 unfamiliar-game rerun is an explicit FAIL, not a smoothness promotion. | [tooling/proof](status/capabilities/tooling-proof.md) |
...
`pnpm verify:template-production`, `pnpm verify:img2threejs`, `pnpm verify:conformance`,
