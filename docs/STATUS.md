...
| Rendering | Active | Authored rendering IR maps through adapter-private implementations; live reconciliation, texel-stable sun-shadow cascades, portable planar contact shadows with bounded static/dynamic cost, calibrated lighting/material/dense evidence, cubemaps, render-layer limits, teardown, residual classification, and focused cross-adapter proof are covered, while raw/backend shader surfaces remain deferred. | [rendering](status/capabilities/rendering.md) |
