...
| Rendering | Active | Authored rendering IR maps to web and native through adapter-private implementations, parity gates with reusable named visual metric bundles, promoted cinematic defaults, web/native-proved scaffold look presets, compiler-derived portable photoreal capability requirements with web/native AO, bloom, DOF, moving-object motion-blur, and wet-floor SSR fixture proof, and bounded portable shader material v1 runtime screenshots/region metrics across web and Bevy while HDRI, SSGI, raw/backend shader surfaces, and broader release policy remain deferred. | [rendering](status/capabilities/rendering.md) |
