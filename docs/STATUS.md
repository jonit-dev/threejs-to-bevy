...
| Physics | Active | Phases 1-2 promote compound colliders, same-tick at-point forces, retained queries, and wheel/tire/surface simulation; Phase 3 implements the bounded pre-release drivetrain/controller contract with paired web/native plus real web/desktop playtest evidence, while release enrollment, aerodynamics, and later phases remain active. | [physics](status/capabilities/physics.md) |
