...
| Rendering | Active | Authored rendering IR maps to web and native through adapter-private implementations, parity gates, promoted cinematic defaults, web/native-proved scaffold look presets, portable photoreal feature report contracts with rollout-gap diagnostics, and bounded portable shader material v1 runtime screenshots/region metrics across web and Bevy while raw/backend shader surfaces remain deferred. | [rendering](status/capabilities/rendering.md) |
