...
| Assets | Active | Bundle-local assets include provider provenance, relocatable material-aware model-test projects with fail-closed duplicate-path diagnosis and repair, secret-isolated experimental model-provider jobs, explicit-consent Linux x64 Blender recipes with policy-preserving atomic reruns, exact generated-animation reconciliation and explicit initial state, plus bounded source-GLB transform animation, authored hinge pivots, decimation, and polygon-preserving split targets; reviewed img2threejs generation retains descriptor-derived CLI/MCP ownership, deterministic reload proof, and native load evidence. Native pixel parity, rigs, broader material/upstream factories, other Blender hosts, and credential-backed provider promotion remain explicit gaps. | [assets](status/capabilities/assets.md) |
| Audio/platform | Active | Web/native bundle-local startup, event, and script playback execute in their game loops with native entity-spawn proof; active logical playback supports bounded absolute volume/pitch updates with matching non-success semantics, while native sink controls execute except seek. A release-enrolled project audit derives sound bindings and playback gain from durable source, measures source/effective intensity, reconciles generated loop intent, and rejects short, edge-imbalanced, or discontinuous loops; generated tones, device routing, and runtime network audio retain explicit boundaries. | [audio/platform](status/capabilities/audio-platform.md) |
