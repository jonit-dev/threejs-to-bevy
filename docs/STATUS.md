...
| Native parity | Frozen for promotion | Bevy consumes emitted IR; the native playtest P0 is closed, the bounded gameplay parity smoke pairs one humanoid movement playtest across web/desktop, trace-only UI rows are not promoted, and new native promotions require shipped-game evidence, native proof, and the native path decision. | [native parity](status/capabilities/native-parity.md) |
