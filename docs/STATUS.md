...
| UI | Active | Retained structured UI is the portable path; one optional React overlay bundle can target web and desktop through typed contracts, capability-checked native launch, runtime input/visibility controls, web/desktop snapshot delivery, and a synchronized transparent Linux GTK/Wry host; transparent WebKitGTK overlays are not promoted on NVIDIA/Xwayland, where affected projects must use retained native UI, alongside cross-compositor lifecycle, IME, virtual keyboards, platform screen readers, native style effects, and rendered world attachment boundaries. | [ui](status/capabilities/ui.md) |
