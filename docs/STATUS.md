...
| UI | Active | Retained structured UI is the portable path; one optional React overlay bundle can target web and Linux desktop through typed contracts, capability-checked native launch, runtime input/visibility controls, snapshot delivery, and a CEF off-screen surface composited into Bevy. NVIDIA/Xwayland chooser, hover, Black-side start, and repeated modal pixels are proved; multi-overlay layout, keyboard/IME/focus, complete input ownership, other compositors, Windows/macOS, platform screen readers, native style effects, and rendered world attachment remain boundaries. | [ui](status/capabilities/ui.md) |
