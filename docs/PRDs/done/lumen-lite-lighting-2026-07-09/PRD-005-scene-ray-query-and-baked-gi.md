...
- [x] Baked data is durable content (`content/lighting/`), regenerated by an
      explicit command; the build fails soft (diagnostic, not error) when
      probes are stale.
- [x] `tn bake gi` enrolls in the CLI command registry first; help/dispatch
      derive from it (repo registry-first rule).
- [x] SceneRayQuery is internal; scripting exposure is a separate future PRD
      with capability gating (users must not get free filesystem-adjacent
      power through it).
- [x] SH2 chosen over cubemaps for the payload: 27 floats, trivially
...
- [x] Seeded hemisphere sampling, sun/sky direct + one albedo bounce, SH
      projection; `--json` report with ray counts, timings, content hash.
- [x] Staleness: content-hash mismatch at build -> diagnostic naming the
      rebake command.
...
- [x] Opens with the SH->EnvironmentMapLight vs IrradianceVolume spike;
      records the decision in this PRD before implementation.

**Phase 5 spike decision (2026-07-10):** Bevy 0.14.2 cannot bind the portable
27-float SH2 payload directly. `EnvironmentMapLight` requires diffuse and
specular cubemap `Image` handles; `IrradianceVolume` requires a specially
packed 3D voxel texture. V1 therefore applies the SH L0 irradiance as a colored
ambient contribution and reports
`global-ambient-sh-l0-approximation`. Generating adapter-private GPU textures
for spatial/native-exact probes remains follow-up work; v1 never reports this
fallback as exact.
...
**Implemented consumer:** Native playtest `assert.occluded` emits a private
`sceneOcclusion` proof-harness command. The Bevy adapter resolves it through a
Rapier/parry `TriMesh` built from rendered mesh assets without consulting
Collider components, and returns `render.sceneRayQuery` evidence to the shared
assertion evaluator. This does not expose SceneRayQuery to authored scripts.
...
- [x] `tn bake gi` produces deterministic, committed SH probe payloads from
      authored scenes; command derives from the CLI registry.
- [x] Both adapters light the alcove fixture from the same payload (native
      may report a documented approximation mode, never silence).
- [x] Staleness is detected and diagnosed, not silently wrong.
- [x] `three-mesh-bvh` is fully wrapped; no upstream type leaks into public
      surfaces; no scene-merge/private-API usage.
- [x] Native rendered-geometry queries use the private parry-backed service,
      and the playtest occlusion consumer does not fall back to collider-only
      physics raycasts.
- [x] rendering.md, STATUS.md, bevy-feature-parity.md updated; PRD moves to
      done when complete.

## Completion Record

Completed 2026-07-10. All six HIGH-mode phases received independent review;
the final review approved the compiler BVH query, deterministic SH2 bake and
staleness contract, web camera-weighted SH2 consumption, native honest SH-L0
ambient approximation, paired baked-GI gate, and private native parry
render-geometry query consumed by playtest occlusion proof. Final focused
evidence includes `pnpm verify:focused verify:baked-gi`, the complete
photoreal matrix, fresh-bake/no-stale cookbook proof, compiler/IR/CLI tests,
and the native render-only mesh query test.
