...
test("break thresholds should emit once and retire the joint observation on the next tick", async () => {
  await initializePhysicsRuntime();
  const world = makeRichJointWorld();
  world.entities = world.entities.filter((entity) => entity.id === "anchor" || entity.id === "fixed");
  const fixed = world.entities.find((entity) => entity.id === "fixed");
  ...
  assert.deepEqual(observePhysicsJointLoads(world).map(({ active, entity, force }) => ({ active, entity, force })), [{ active: true, entity: "fixed", force: 1000 }]);
  ...
  assert.deepEqual(observePhysicsJointLoads(world).map(({ active, entity, force }) => ({ active, entity, force })), [{ active: false, entity: "fixed", force: 1000 }]);
  assert.deepEqual(physicsRuntimeStats(world), { jointCreations: 1, jointRemovals: 1, rebuilds: 1 });
  disposePhysicsRuntime(world);
});
