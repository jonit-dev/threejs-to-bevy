...
test("rendering should validate v2 point light fixture", async () => {
  const root = await mkdtemp(join(tmpdir(), "tn-rendering-light-"));
  try {
      ...
      entities: [
        {
          id: "light.point",
          components: {
            Light: { kind: "point", color: "#ffffff", intensity: 2 },
...
test("rendering should reject unsupported shader material", async () => {
  const root = await mkdtemp(join(tmpdir(), "tn-rendering-shader-"));
  try {
    await writeBaseBundle(root);
    await writeJson(root, "materials.ir.json", {
      schema: "threenative.materials",
      version: "0.1.0",
      materials: [{ id: "mat.shader", kind: "shader", color: "#ffffff", shader: "custom" }],
    });

    const result = await validateBundle(root);

    assert.equal(result.ok, false);
    assert.equal(result.diagnostics[0]?.code, "TN_IR_MATERIAL_UNSUPPORTED");
    assert.equal(result.diagnostics[1]?.code, "TN_IR_MATERIAL_CAPABILITY_UNSUPPORTED");
  } finally {
    await rm(root, { force: true, recursive: true });
  }
});
