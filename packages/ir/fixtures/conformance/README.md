...
| `v8-retained-ui-disabled` | `rendering:camera.perspective`, `ui:accessibility`, `ui:action`, `ui:disabled`, `ui:focus-order`, `ui:node.button`, `ui:node.column`, `ui:node.stack`, `ui:runtime` | V8 retained UI fixture proving authored static disabled controls remain in the semantic tree while being removed from initial focus/action dispatch in web and Bevy. |
