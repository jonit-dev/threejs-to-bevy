...
| --- | --- | --- |
| `advanced-physics-foundation` | `physics:collider.compound`, `physics:force-at-point`, `physics:impulse-at-point`, `physics:query.retained`, `physics:rigid-body.dynamic` | Executable paired script-host fixture for exact retained compound-child queries, same-tick off-center force/impulse motion, event order, causal no-command controls, and source/bundle staleness provenance. |
| `advanced-physics-wheels` | `physics:surface`, `physics:tire-model`, `physics:wheel-assembly` | Paired static-load and asphalt/ice wheel fixture for authored wheel order, grounded/surface semantics, registry-toleranced suspension/tire observations, and the non-driven causal control. |
