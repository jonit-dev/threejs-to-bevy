...
test("should print help when requested", () => {
...
test("should tolerate a leading package script separator", () => {
  const result = dispatch(["--", "--help"]);
...
test("should return structured placeholder output for future commands", () => {
  const result = dispatch(["build", "--json"]);
