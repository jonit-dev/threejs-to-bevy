...
test("should emit ui ir for scene with portable hud", async () => {
  ...
  try {
    ...
    const bundlePath = await emitBundle(config, {
      ...
      ui: Ui({
        children: Column({
          ...
          id: "hud.stack",
          layout: { inset: { left: 24, top: 16 }, overflow: "hidden", position: "absolute", zIndex: 5 },
        }),
        id: "hud",
      }),
    });
    ...
    assert.deepEqual(ui.root.children[0].layout, { inset: { left: 24, top: 16 }, overflow: "hidden", position: "absolute", zIndex: 5 });
