...
import type { IScriptModuleGraph } from "./moduleGraph.js";
import { bundleLocalScriptModules } from "./moduleBundler.js";
...
export interface ISystemScriptSource {
  ...
  script?: {
    ...
    localModuleGraph?: IScriptModuleGraph;
  };
...
export function bundleSystemScripts(systems: ReadonlyArray<ISystemScriptSource>): IScriptBundleResult {
  ...
  const declarations = scriptedSystems.map((system) => {
    const localEntry = localModuleBundle.entries.get(system.script.exportName);
    return localEntry === undefined
      ? `const ${system.script.exportName} = ${normalizeSystemSource(system.script.source ?? "")};`
      : `const ${system.script.exportName} = ${localEntry};`;
  });
  ...
  return {
    ...
    code: [
      ...
      ...helperDeclarations,
      ...handles,
      ...(localModuleBundle.code.length === 0 ? [] : [localModuleBundle.code]),
      ...declarations,
      ...
      ...(system.script.sourceRef === undefined
        ...
        : {
            source: {
              ...
              ...(system.script.localModuleGraph === undefined ? {} : { moduleGraph: compactModuleGraph(system.script.localModuleGraph) }),
            },
          }),
