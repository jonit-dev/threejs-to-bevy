...
impl ViewNode for NativeSsgiNode {
    type ViewQuery = (Entity, &'static ViewTarget, &'static ViewDepthTexture, &'static DynamicUniformIndex<NativeSsgi>, &'static ViewUniformOffset);
    fn run(&self, _graph: &mut RenderGraphContext, context: &mut RenderContext, (view_entity, target, depth, settings_index, view_offset): QueryItem<Self::ViewQuery>, world: &World) -> Result<(), NodeRunError> {
        ...
        let mut histories = world.resource::<NativeSsgiHistory>().entries.lock().expect("native SSGI history mutex should not be poisoned");
        let history = histories.entry(view_entity).or_insert_with(|| NativeSsgiHistoryEntry::new(context.render_device(), source.width(), source.height(), target.main_texture_format()));
        if history.width != source.width() || history.height != source.height() {
            *history = NativeSsgiHistoryEntry::new(context.render_device(), source.width(), source.height(), target.main_texture_format());
        }
        ...
        let layout = if multisampled { &pipeline.multisampled_layout } else { &pipeline.layout };
        {
            ...
            let bind_group = context.render_device().create_bind_group("native_ssgi_bind_group", layout, &BindGroupEntries::sequential((&sampleable_source_view, &pipeline.sampler, depth.view(), &history.view, settings_binding, view_binding)));
...
impl NativeSsgiHistoryEntry {
    fn new(device: &RenderDevice, width: u32, height: u32, format: bevy::render::render_resource::TextureFormat) -> Self {
        ...
        Self { texture, view, width, height, initialized: false }
    }
}
...
fn create_layout(device: &RenderDevice, multisampled: bool) -> BindGroupLayout {
    ...
    device.create_bind_group_layout("native_ssgi_layout", &BindGroupLayoutEntries::sequential(ShaderStages::FRAGMENT, (texture_2d(bevy::render::render_resource::TextureSampleType::Float { filterable: true }), sampler(SamplerBindingType::Filtering), depth, texture_2d(bevy::render::render_resource::TextureSampleType::Float { filterable: true }), uniform_buffer::<NativeSsgi>(true), uniform_buffer::<ViewUniform>(true))))
}
