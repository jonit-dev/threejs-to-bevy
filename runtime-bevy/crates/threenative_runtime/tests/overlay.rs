...
fn routes_native_overlay_events_both_directions_through_world_queues() {
    let overlays = make_overlays();
    let mut bridge = NativeOverlayBridge::new();
    assert!(bridge.receive_overlay_message(&overlays, "inventory", "inventory:use-item", json!({ "itemId": "potion" })));
    let mut world_events = HashMap::new();
    bridge.drain_events_into(&mut world_events);
    assert_eq!(world_events["inventory:use-item"], json!([{ "itemId": "potion" }]));

    world_events.insert("inventory:snapshot".to_owned(), json!([{ "gold": 9 }]));
    bridge.publish_world_events(&overlays, &world_events);
    assert_eq!(bridge.snapshots().back().map(|entry| &entry.payload), Some(&json!({ "gold": 9 })));
}
...
fn validates_shared_overlay_payload_conformance_vectors() {
    let fixture_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../../packages/ir/fixtures/overlay-payload-validation.json");
    let fixture: serde_json::Value = serde_json::from_str(&fs::read_to_string(fixture_path).expect("shared overlay vectors")).expect("valid vectors json");
    let schema = &fixture["schema"];
    let fields = schema["fields"].as_object().expect("schema fields").iter().map(|(name, kind)| (name.clone(), kind.as_str().expect("kind").to_owned())).collect();
    let required = schema["required"].as_array().expect("required").iter().map(|value| value.as_str().expect("required name").to_owned()).collect();
    let overlays = OverlaysIr {
        schema: "threenative.overlays".to_owned(),
        version: "0.2.0".to_owned(),
        overlays: vec![OverlayIr {
            id: "vectors".to_owned(), entry: "overlay/index.html".to_owned(), transparent: true, z_index: 1, input: "none".to_owned(),
            layout: None,
            messages: OverlayBridgeMessagesIr { overlay_to_game: vec![], game_to_overlay: vec![OverlayMessageIr { name: "vectors:snapshot".to_owned(), schema: OverlayMessageSchemaIr { kind: "object".to_owned(), fields, required } }] },
    ...
    for vector in fixture["vectors"].as_array().expect("vectors") {
        ...
        if let Some(bytes) = vector["paddingBytes"].as_u64() {
            payload.as_object_mut().expect("payload object").insert("label".to_owned(), json!("x".repeat(bytes as usize)));
        }
        let mut bridge = NativeOverlayBridge::new();
        let accepted = bridge.publish_game_message(&overlays, "vectors", "vectors:snapshot", payload);
        assert_eq!(accepted, vector["valid"].as_bool().expect("valid"), "{}", vector["name"]);
        if !accepted {
            assert_eq!(bridge.diagnostics().last().map(|diagnostic| diagnostic.code.as_str()), vector["code"].as_str());
        }
    }
}
