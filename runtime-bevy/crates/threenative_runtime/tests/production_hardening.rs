...
fn should_report_production_audio_profiler_debug_and_boundaries() {
    ...
    assert_eq!(report["audio"]["mixer"]["effects"].as_array().unwrap().len(), 4);
    ...
    assert!(report["boundaries"].as_array().unwrap().iter().any(|boundary| boundary["code"] == "TN_AUDIO_CUSTOM_DECODER_UNSUPPORTED"));
}
