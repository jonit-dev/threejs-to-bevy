...
fn should_report_rendering_material_geometry_and_asset_residuals() {
    ...
    assert_eq!(report["geometry"]["lod"][0]["selectedMesh"], "mesh.hero.low");
    assert_eq!(report["materials"]["specular"][0]["texture"], "texture.specular");
    assert!(report["boundaries"].as_array().unwrap().iter().any(|boundary| boundary["code"] == "TN_RENDERER_CUSTOM_SHADER_UNSUPPORTED"));
}
